...
# このアプリでは date は内部処理の軸でもあるので「必須列」として扱う
MANDATORY_COLS = ["date"]
...
COLUMN_LABELS = {
    ...
    # 最高値（既存表示：この app では phase2_high を “最高値” として扱う）
...
PRESET_COLUMNS = {
    ...
    '分析': ['date', 'symbol', 'move_vs_entry', 'low_move_vs_entry', 'reach_time', 'low_reach_time', 'skip_minutes', 'weekday_jp', 'detail'],
    '詳細': [
        'date', 'symbol',
        'entry',
        'target_price', 'reach_time', 'move_vs_entry',
        'low_target_price', 'low_reach_time', 'low_move_vs_entry',
        'used_tier_index', 'used_mm_rate', 'skip_minutes', 'detail'
    ],
}
...
def build_detail_view_dataframe(results_df, source_df):
    ...
    # 最高値（phase2_high を使用）
    # ==========
    high_price = pd.to_numeric(merged_df.get('phase2_high'), errors='coerce')
    entry_price = pd.to_numeric(merged_df.get('entry'), errors='coerce')

    merged_df['target_price'] = high_price
    merged_df['reach_time'] = merged_df.get('phase2_high_time')
    merged_df['move_vs_entry'] = high_price - entry_price

    # ==========
    # 最底値（phase2_low を使用）
    # ==========
    low_price = pd.to_numeric(merged_df.get('phase2_low'), errors='coerce')
    merged_df['low_target_price'] = low_price
    merged_df['low_reach_time'] = merged_df.get('phase2_low_time')
    merged_df['low_move_vs_entry'] = low_price - entry_price
...
def apply_move_color(styled, display_df, move_label):
    if move_label in display_df.columns:
        if hasattr(styled, "map"):
            styled = styled.map(
                lambda value: 'color: #1976D2' if str(value).startswith('+')
                else 'color: #D32F2F' if str(value).startswith('-')
                else '',
                subset=[move_label],
            )
        else:
...
try:
    ...
    with col_liq1:
        st.metric("基本", f"{liq_distance_pct_base * 100:.3f}%", help="追加証拠金なしの場合")
    ...
    if view == "📅 カレンダー表示":
        ...
        if len(results) == 0:
            st.warning("データがありません。先に build_daily_aggregates.py を実行してください。")
        else:
    ...
    # -------------------------
        ...
        # 必須列はトグルに出さない
        col_candidates = [
            c for c in COLUMN_LABELS.keys()
            if c in detail_df.columns and c not in MANDATORY_COLS
        ]

        # ---- すべてのwidgetに安定keyを付ける（ここが重要）----
        VISIBLE_KEY = "visible_optional_cols_v3"
        LAST_PRESET_KEY = "last_preset_v3"

        SORT_KEY = "detail_sort_key_v3"
        SORT_ASC_KEY = "detail_sort_asc_v3"

        SYM_KEY = "detail_symbols_v3"
        WD_KEY = "detail_weekdays_v3"
        SKIP_KEY = "detail_skip_only_v3"
        LOSS_KEY = "detail_loss_only_v3"
        TIER_KEY = "detail_tier_range_v3"

        # 初回：プリセット反映（optionalのみ）
        if VISIBLE_KEY not in st.session_state:
            st.session_state[VISIBLE_KEY] = [c for c in preset_cols if c not in MANDATORY_COLS and c in col_candidates]
            st.session_state[LAST_PRESET_KEY] = preset

        # プリセット変更時：選択を差し替えて rerun
        if st.session_state.get(LAST_PRESET_KEY) != preset:
            st.session_state[VISIBLE_KEY] = [c for c in preset_cols if c not in MANDATORY_COLS and c in col_candidates]
            st.session_state[LAST_PRESET_KEY] = preset
            st.rerun()

        # リセット（列トグルだけじゃなく、関連stateも消す）
        if st.button("表示列トグルをリセット", key="reset_visible_cols_v3", help="選択状態が壊れた時に押してください"):
            for k in [
                VISIBLE_KEY, LAST_PRESET_KEY,
                SORT_KEY, SORT_ASC_KEY,
                SYM_KEY, WD_KEY, SKIP_KEY, LOSS_KEY, TIER_KEY,
            ]:
                st.session_state.pop(k, None)
            st.rerun()

        # options変動に合わせて state を正規化（重要）
        current = st.session_state.get(VISIBLE_KEY, [])
        normalized = [c for c in current if c in col_candidates]
        if normalized != current:
            st.session_state[VISIBLE_KEY] = normalized

        selected_optional_cols = st.multiselect(
            '表示列トグル',
            options=col_candidates,
            format_func=lambda c: COLUMN_LABELS.get(c, c),
            key=VISIBLE_KEY,  # optionalだけ保持
        )

        # 表示用：必須列を先頭に差し込む
        visible_cols = [*MANDATORY_COLS, *list(selected_optional_cols)]
        visible_cols = list(dict.fromkeys(visible_cols))  # 順序維持で重複除去
        ...
        st.markdown('#### フィルタ')
        with st.form(f"detail_filters_{preset}", clear_on_submit=False):
            filter_col1, filter_col2, filter_col3 = st.columns(3)

            symbols = sorted(detail_df.get('symbol', pd.Series(dtype='object')).dropna().unique().tolist())
            selected_symbols = filter_col1.multiselect('シンボル', options=symbols, default=symbols, key=SYM_KEY)
            ...
            selected_weekdays = filter_col2.multiselect('曜日', options=WEEKDAY_ORDER, default=available_weekdays, key=WD_KEY)

            skip_only = filter_col3.checkbox('skip_minutes > 0 のみ', key=SKIP_KEY)
            loss_only = filter_col3.checkbox('ロスカット発生日のみ', key=LOSS_KEY)

            # used_tier_index 安全版（列が無ければ空Series）
            ...
            _ = st.form_submit_button("フィルタ適用")
        ...
        # ソート（キーを付けて安定化）
        sort_options = [c for c in (MANDATORY_COLS + col_candidates) if c != 'detail']
        ...
        sort_asc = st.checkbox('昇順', value=False, key=SORT_ASC_KEY)
        ...
        styled = apply_move_color(styled, display_df, COLUMN_LABELS['move_vs_entry'])
        styled = apply_move_color(styled, display_df, COLUMN_LABELS['low_move_vs_entry'])
        ...
        # 表の列構成が変わったら “別key” にして、列ドラッグの状態を引き継がない
        _cols_sig = "|".join(display_df.columns)
        _table_key = "detail_table_" + hashlib.md5(_cols_sig.encode("utf-8")).hexdigest()[:10]
        
        st.dataframe(styled, use_container_width=True, height=600, key=_table_key)

"""
判定ロジック：日次集計データから判定結果を生成（高速化版）

※ データは事前にフィルタ済みなので、パラメータチェックは不要
"""
...
def judge_day(row, liq_model, leverage, position_margin, additional_margin=0, df_1min=None):
    ...
    注意:
        この関数は既にフィルタ済みのデータを受け取るので、
        threshold_min と judgment_hours のチェックは不要
    """
    date = row['date']
...
def judge_all(df_aggregates, liq_model, leverage, position_margin, additional_margin=0,
    ...
    注意:
        df_aggregates は既にフィルタ済み（threshold_min と judgment_hours が一致）
        なので、全件処理する

    Args:
        df_aggregates: フィルタ済みの日次集計データ
    ...
    for i, (idx, row) in enumerate(df_aggregates.iterrows()):
        result = judge_day(row, liq_model, leverage, position_margin, additional_margin, df_1min)
